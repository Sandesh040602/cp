...
class SegTree {
    ...
    Node mergeFunc(Node& a, Node& b) {
        Node ans;
        ans.data = min(a.data, b.data);
        return ans;
    }

    void buildSegTree(vector<int>&v, int ind, int l, int r) {
        cout<<l<<"---"<<r<<"---"<<ind<<endl;
        if (l==r) {
            segNodes[ind].data = v[l];
            return;
        }

        int mid = l + (r - l)/2;
        buildSegTree(v, 2*ind + 1, l, mid);
        buildSegTree(v, 2*ind + 2, mid+1, r);
        dbg(l); dbg(r); dbg(ind);

        segNodes[ind] = mergeFunc(segNodes[2*ind + 1], segNodes[2*ind + 2]);
    }
    ...
    void buildSegTree(vector<int>&v) {
        dbg("reached");
        int segSize = 4*v.size();
        dbg(segSize)
        segNodes = vector<Node>(segSize);
        buildSegTree(v, 0, 0, v.size()-1);
        for(auto it: segNodes) cout<<it.data<<"::";
        cout<<endl;
    }

    Node query(int ind, int l, int r, int cl, int cr) {
        if (l<=cl && cr<=r) return segNodes[ind];

        Node ans;
        ans.data = INT_MAX;

        if (cr<l || r<cl) return ans;

        int mid = cl + (cr - cl)/2;
        Node a = query(2*ind + 1, l, r, cl, mid);
        Node b = query(2*ind + 2, l, r, mid+1, cr);
...
void solve() {
    ...
    while (q--) {
        int l, r;
        cin>>l>>r;
        cout<<s.query(0, l, r, 0, n-1).data<<endl;;
    }
