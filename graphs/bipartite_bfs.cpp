...
// Functions
bool helper(int node, vector<vector<int>>&edges, vector<int>&vis){
    bool cans = 1;
    vis[node] = 1;
    queue<int>q;
    q.push(node);
    while(q.size()){
        ...
        for(int i=0;i<edges[curr].size();i++){
            if(vis[edges[curr][i]]!=-1 && vis[edges[curr][i]]==vis[curr]){return false;}
            else if(vis[edges[curr][i]]==-1){
                vis[edges[curr][i]] = 1 - vis[curr];
                q.push(edges[curr][i]);
            }
        }
    }
...
bool helper1(int node, vector<vector<int>>&edges, vector<int>&vis, int dest){
    if(vis[node]!=-1){
        return vis[node]==dest;
    }
    vis[node] = dest;
    bool cans = 1;
    for(int i=0;i<edges[node].size();i++){
        if(vis[edges[node][i]]!=-1){
            cans&=vis[edges[node][i]]!=dest;
        }
        else{
            cans&=helper1(edges[node][i], edges, vis, 1 - dest);
        }
    }
    return cans;
}
...
void solve(){
    ...
    for(int i=0;i<n;i++){
        if(vis[i] == -1){
            dbg(i);
            cans&=helper1(i, edges, vis, 0);
            dbg(vis);
        }
    }
    if(cans){cout<<"bipartite posssible"<<endl;}
