...
// Functions
bool helper_bfs(int node, vector<vector<int>>&edges, vector<int>&vis, int par){
    queue<pair<int,int>>q;
    q.push(make_pair(node, par));
    vis[node] = 1;
    while(q.size()){
        pair<int,int>curr = q.front(); q.pop();
        dbg(curr)
        for(int i=0;i<edges[curr.first].size();i++){
            if(vis[edges[curr.f][i]]){
                if(edges[curr.f][i] == curr.second){continue;}
                else{return true;}
            }
            else{
                pair<int,int>child(make_pair(edges[curr.first][i],curr.f));
                q.push(child);
                vis[child.f] = 1;
            }
        }
...
bool helper_dfs(int node, vector<vector<int>>&edges, vector<int>&vis, int par){
    vis[node] = 1;
    bool cans = 0;
    for(int i=0;i<edges[node].size();i++){
        if(vis[edges[node][i]]==1){
            if(edges[node][i]!=par){return true;}
        }
        else{
            vis[edges[node][i]] = 1;
            cans|=helper_dfs(edges[node][i], edges, vis, node);
        }
    }
    return cans;
}
...
void solve(){
    ...
    for( int i = 0; i<n ; i++){
        if(visited[i]==0){
            cout<<"entered"<<endl;
            // bool cans = helper_bfs(i, edges, visited, -1);
            bool cans = helper_dfs(i, edges, visited, -1);
            ans |= cans;
        }
    }
